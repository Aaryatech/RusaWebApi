...
public interface EventCount extends JpaRepository<EventCountDetails, Integer>{
	  ...
	  @Query(value="select nh.*,nd.heading,nd.descriptions,nd.language_id,nd.page_meta_title,nd.page_meta_description,nd.page_meta_keyword,\r\n" + 
			  		...
			  		+ "from t_newsblogs nh,t_newsblogs_description nd where nd.newsblogs_id=nh.newsblogs_id and nd.language_id=1 and nh.del_status=1 and nh.is_active=1 and nh.ex_int1=11 and nh.event_date_from BETWEEN (:fromDate) AND (:toDate)",nativeQuery=true)
	List<EventCountDetails> getAllEvents(@Param("fromDate")String fromDate,@Param("toDate")String toDate);  
}
